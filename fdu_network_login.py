...
class Srun:
    def __init__(self, host, username, password, interval):
        ...
        self.password = '{TEXT}' + password # FDU PLAIN TEXT VERSION, but standard srun3000 should be "hashlib.md5(password).hexdigest()[8:-8]"
...
def main():
    while True:
        try:
            srun = Srun("{login server ip}", "{fdu_username}", "{fdu_pass}", 30)
            #srun = Srun("10.108.255.249", "123username", "456password", 30)
            srun.run()
        except Exception, e:
            print e
            time.sleep(10)
